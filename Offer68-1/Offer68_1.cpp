...
BinaryTreeNode* lowestCommonAncestor(BinaryTreeNode* root, BinaryTreeNode* p, BinaryTreeNode* q) {
	if (root == nullptr)
		return nullptr;
	if ((root->m_nValue < p->m_nValue&&root->m_nValue > q->m_nValue)|| (root->m_nValue > p->m_nValue&&root->m_nValue < q->m_nValue))
	{
		return root;
	}
	...
	{
		return lowestCommonAncestor(root->m_pRight, p, q);
	}
	else if (root->m_nValue > p->m_nValue && root->m_nValue > q->m_nValue)
	{
		return lowestCommonAncestor(root->m_pLeft, p, q);
	}
	return nullptr;
}
...
*/
	...
	BinaryTreeNode* pNode3 = CreateBinaryTreeNode(8);
	...
	BinaryTreeNode* pNode7 = CreateBinaryTreeNode(3);
	...
	auto output = lowestCommonAncestor(pNode1, pNode4, pNode7);
